...
void test_dynamic_array() {
        ...
        DynamicArray da = DA_create(sizeof(a));
        assert(da.data != 0);
        assert(da.count == 0);
...
void test_freelist() {
        ...
        FreeList fl = FL_create(sizeof(a));
        assert(fl.data != 0);
        assert(fl.occup != 0);
        ...
        struct example* reused_elem2 = (struct example*) FL_at(&fl, reused_idx2);
        ...
        struct example* reused_elem3 = (struct example*) FL_at(&fl, reused_idx3);
...
void test_slotmap() {
        ...
        SlotMap sm = SM_create(sizeof(a));
        assert(sm.index_map.data != 0);
        assert(sm.data.data != 0);
        assert(sm.data.count == 0);
        ...
        for (int i = 0; i < 10; i++) {
                SM_id idx = SM_get_index(&sm, ids[i]);
                assert(idx < sm.data.count);
        }

        // Test SM_remove_at_id with a non-swap remove
        // Remove element with x=0 (ids[0])
        SM_remove_at_id(&sm, ids[0]);
        assert(sm.data.count == 9);
        ...
        for (int i = 1; i < 10; i++) {
                ...
                SM_remove_swap_at_id(&sm, ids[i]);
        }
        SM_remove_swap_at_id(&sm, new_id);
        assert(sm.data.count == 0);
        ...
        assert(sm.data.count == 5);

        for (int i = 0; i < 5; i++) {
                struct example* elem = (struct example*) SM_at_id(&sm, fresh_ids[i]);
                assert(elem->x == i + 200);
        ...
        for (int i = 0; i < 5; i++) {
                SM_remove_at_id(&sm, fresh_ids[i]);
                assert(sm.data.count == (size_t) (4 - i));
        }
